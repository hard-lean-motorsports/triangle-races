function [interp, interp_arr, val, weight_arr] = extract_vd_info(vd_table, index)
    ...
    if(isnan(vd_table{1, index}))
        interp = 1;
        interp_arr = nan_end_extract(vd_table{1:end, index});
        weight_f = nan_end_extract(vd_table{1:end, index+1});
        weight_r = nan_end_extract(vd_table{1:end, index+2});
        weight_s = nan_end_extract(vd_table{1:end, index+3});
